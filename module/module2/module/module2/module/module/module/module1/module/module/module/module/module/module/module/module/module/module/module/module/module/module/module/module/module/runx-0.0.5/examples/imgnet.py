...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.gpu is not None:
        logx.msg(f"Use GPU: {args.gpu} for training")
    ...
    args.arch = 'resnet18'
    logx.msg(f"=> creating model '{args.arch}'")
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            logx.msg(f"=> loading checkpoint '{args.resume}'")
            checkpoint = torch.load(args.resume)
            args.start_epoch = checkpoint['epoch']
            best_acc1 = checkpoint['best_acc1']
            model.load_state_dict(checkpoint['state_dict'])
            optimizer.load_state_dict(checkpoint['optimizer'])
            logx.msg(f"=> loaded checkpoint '{args.resume}' (epoch {checkpoint['epoch']})")
        else:
            logx.msg(f"=> no checkpoint found at '{args.resume}'")
